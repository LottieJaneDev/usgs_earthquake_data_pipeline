...
2. Start the Docker container using the [`docker-compose.yaml`](usgs_earthquake_data_pipeline/docker-compose.yaml) configurations & the subsequent Mager [`Dockerfile`](usgs_earthquake_data_pipeline/mage/Dockerfile) to build the container image. 
...
> Alternatively, I have set my Google Cloud Storage bucket to public [here](https://console.cloud.google.com/storage/browser/usgs-raw-data;tab=objects?forceOnBucketsSortingFiltering=true&authuser=1&project=usgs-earthquake-data) you can read this into your BigQuery table using a `SQL Query` tab if you wish to.
